...
namespace Etienne.Pools
{
    public class ComponentPool<T> : Pool<T> where T : Component
    {
        ...
        {
            Inspector.EnqueueAfterDelay(this, item, .1f);
        }

        public void DelayedEnqueue(T item)
        {
            if (item == null || queue.Contains(item)) return;
            Inspector.UnStickTransform(item.transform);
        ...
        private void EnqueueNewItem(int index)
        {
            if (prefab == null)
            {
                ...
                DelayedEnqueue(go.AddComponent<T>());
                go.transform.SetParent(Inspector.transform, false);
            }
            else
            {
                ...
                DelayedEnqueue(instance);
                instance.transform.SetParent(Inspector.transform, false);
            }
        }
    }
    public class ComponentPoolInspector : MonoBehaviour
    {
        ...
        Dictionary<Transform, Transform> items = new Dictionary<Transform, Transform>();
        ...
        public void StickToTransform(Transform child, Transform parent)
        {
            items.Add(child, parent);
        }

        public void UnStickTransform(Transform child)
        {
            items.Remove(child);
        }
